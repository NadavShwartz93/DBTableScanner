...
def change_dir(filename='config.ini'):
    ...
    in order to save all the excel files in this directory.
    """
    config = ConfigParser()
    config.read(filename)
...
def file_name_valid(file_name):
    """The function check if the file name is valid."""
    if file_name == '':
        return 'hello.xlsx'
    elif not str(file_name).endswith('.xlsx'):
        temp_name = "".join((file_name, '.xlsx'))
        return temp_name
    else:
        return file_name
